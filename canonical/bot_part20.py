stop_pips = abs(sig['entry_price'] - sig['stop']) / pip
            ...
            if force_preview:
                ...
                msg += f"\n"゠любласты — ☃ 🚏🎳👼 — {pd.Timestamp.now('UTC').strftime('%Y-%m-%d %H:%M')} UTC– последний реальный сигнал, бот его НЕ открывает_"
                for cid in CHAT_IDS:
            ...
            if stop_pips > MAX_STOP_PIPS:
                skip_msg = format_skip_message(sig, f" пипс > лимит {MAX_STOP_PIPS}")
                for cid in CHAT_IDS:
            ...
            if risk_pct > MAX_RISK_PCT:
                skip_msg = format_skip_message(sig, f"иск {risk_pct*100:.1f}% > лимит {MAX_RISK_PCT}")
                for cid in CHAT_IDS:
                    send_message(TELEGRAM_TOKEN, cid, skip_msg)
                continue
            # Open position
            ...
            new_pos = {
                ...
                'entry_price': sig['entry_price'] + (spread_price(pair)\/
                 /2 if sig['direction']=='long' else -spread_price(pair)\/
                 /2),
